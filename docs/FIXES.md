...
审计时仓库共有 3 次提交、1 个分支（`main`）、无 tag：

| 次序 | 日期 | 说明 |
| --- | --- | --- |
| 第 1 次提交 | 2026-06-29 | 初始化 |
| 第 2 次提交 | 2026-09-21 | 修复表达式解析与结果验证；补充测试、文档与架构图 |
| 第 3 次提交 | 2026-09-21 | 按复审意见修正文档表述与测试示例 |
...
除密钥之外，审计还发现历史里留有**两项与安全无关、但属于个人数据**的内容：

1. **`learning_log.txt` 曾被提交进公开仓库**（第 1 次提交），
   含完整题目文本与完整原始 LLM 响应（包括接口返回的 `id` 与 token 用量）。
   其中不含密钥。
2. **第 1 次提交使用的作者邮箱是一个真实个人邮箱**
   （本文不重复写出该地址）。此后两次提交均已使用
   `eeeviann@users.noreply.github.com`。

**处理（2026-09-21）：对全部历史执行了一次重写，把上述两项一并消除。**

```bash
pip install git-filter-repo
git filter-repo --mailmap <mailmap> --force      # 旧身份 → noreply 身份
git push --force-with-lease origin main
```

结果与代价：

- 三次提交的作者与提交者身份**全部统一**为
  `eeeviann <eeeviann@users.noreply.github.com>`；
  顺带修正了第 1 次提交里作者名的拼写错误。
- **文件内容零改动。** 重写前后三次提交的 tree 哈希完全一致，
  即只改了提交元数据，没有碰任何一行代码或文档
  （这也是验证"重写没弄坏东西"最直接的方式）。
- 操作前做了完整备份（`git bundle create --all`），
  并实际验证过可以从备份克隆还原出原来的历史。
- **所有 commit hash 因此改变**，这是无法避免的代价。
  本仓库当时 0 fork / 0 star / 0 watch / 0 issue，
  所以没有影响任何协作者或外部引用。

> **但要注意：重写 ≠ 彻底消失。**
> GitHub 会把不再被任何分支或 tag 引用的提交保留一段时间
> （官方说明是"数周"），期间只要知道旧提交的 SHA，仍可直接访问到它。
> 要彻底清除，需要**联系 GitHub 支持**删除缓存视图。
> 官方对这类问题的立场是：**推送过的数据应当视为已经公开**——
> 所以"删掉"从来不等于"没发生"。
>
> 这也正是本项目把那批真实调用记录**主动**整理成
> `examples/real_llm_run_before_fix.txt` 公开的原因：
> 与其假装它不存在，不如把它的来源和含义写清楚。
